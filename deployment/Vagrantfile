...
Vagrant.configure("2") do |config|
  ...
  config.vm.network "forwarded_port", guest: 6600, host: 6600, host_ip: "127.0.0.1"
   ...
   config.vm.provision "shell", inline: <<-SHELL
      ...
      source /opt/deezer/app/venv/bin/activate && pip install -U yt-dlp && deactivate
      ...
      sed -i 's,.*command = /usr/bin/yt-dlp.*,command = /opt/deezer/app/venv/bin/yt-dlp,' /opt/deezer/app/settings.ini
