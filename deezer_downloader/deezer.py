...
def download_song(song: dict, output_file: str) -> None:
    ...
    try:
        ...
        write_id3_metadata(output_file, song, is_flac)
    except MutagenError as e:
        print(f"Could not write metdata to file: {e}")
    except Exception as e:
        raise DeezerApiException(f"Could not write song to disk: {e}") from e
    print("Download finished: {}".format(output_file))


def write_id3_metadata(output_file: str, song: dict, is_flac: bool) -> None:
    ...
    def set_metadata(audio, key, value):
        if not value:
            ...
            if key == 'artist':
                ...
                audio['TDRC'] = TDRC(encoding=3, text=value[:4])
            elif key == 'picture':
            ...
            if key == 'picture':
                ...
                # pic.type = int(PictureType.COVER_FRONT), this raises, even though it shouldn't ...
                pic.desc = 'Cover'
                pic.data = value
    ...
    set_metadata(audio, "date", album_Data.get("PHYSICAL_RELEASE_DATE", None))
